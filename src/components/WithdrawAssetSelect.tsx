...
export const WithdrawAssetSelect: React.FC = () => {
  const { chain, hasSomeRelayerAvailable } = useChainContext();
  ...
  const handleChange = (_event: React.SyntheticEvent, newValue: Option | null) => {
    if (newValue) {
      // Just open the withdraw modal without changing the global selected asset
      // The withdraw modal will handle asset selection internally
...
const StyledWithdrawAutocomplete = styled(Autocomplete<Option, false, true, false>)(({ theme }) => ({
  minWidth: '140px',
...
const WithdrawLabel = styled('span')(({ theme }) => ({
  color: theme.palette.common.white,
  fontWeight: 500,
  marginRight: '4px',
  marginLeft: '8px',
